...
public class CreditScroller : MonoBehaviour
{
    ...
    void Update()
    {
        if (letsGo && !touchInside && (Mathf.Abs(srComponent.velocity.y - 0) <= 0.1))
        {
            letsGo = false;
            PressReleased();
        }
...
// #endif
    ...
    void InitialEaseIn()
    {
        ...
        //isAutoScrolling = true;
        DOTween.To(() => tweenFloat, x => tweenFloat = x, easeInA, 1).SetEase(Ease.InQuad).OnComplete(InitialScroll).SetId("ScrollPro");
    }

    void InitialScroll()
    {
        tweenFloat = easeInA;
        DOTween.To(() => tweenFloat, x => tweenFloat = x, 0, time).SetEase(Ease.Linear).OnComplete(AutoScrollEnd).SetId("ScrollPro");
    }
    ...
    void PressReleased()
    {
        ...
        if (tweenFloat != 0)
        {
            DOTween.To(() => tweenFloat, x => tweenFloat = x, easeStart, 1).SetEase(Ease.InQuad).OnComplete(AutoScrolling).SetId("ScrollPro");
        }
    }
